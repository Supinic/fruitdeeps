...
export class CalcOutputMultiple extends Component {
	...
	render () {
		...
		const dps = playerList.map((player, i) => {
			...
			return (<div key={i} className="flex-child">
				<h2>
					Set {i + 1}
		...
		for (let i = 0; i < playerList.length; i++) {
			if (!(i % 2)) {
				outputBlocks.push(<div className="flex-container top-level">
		...
		return (<div className="flex-container-vertical">
				{outputBlocks}
				{false ? (<CalcOutputTtkChart
						data={this.state.ttkManager.map((ttk) => ttk.ttk.ttkList)}
					/>) : null}
				<div>
