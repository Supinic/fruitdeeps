 ...
 * @returns {Object}
 *
 * @todo refactor param `state` to `player` and `monster` - requires refactoring in `Accuracy.js`
 */
export default function applySpecialAttack (specialName, dps, state) {
	switch (specialName) {
		...
		case "Descent of Dragons": return applyDarkBow(dps, state, { useDragonArrows: true });
		case "Descent of Darkness": return applyDarkBow(dps, state, { useDragonArrows: false });
