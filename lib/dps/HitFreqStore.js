...
export class HitFreqStore {
	...
	static multiplySingle (hitStore, multiplier) {
		if (hitStore.#hitAmount !== 1) {
			throw new Error("Cannot multiply a multi-hit frequency store");
		}

		const newHitStore = new HitFreqStore(1).setZeroAccuracy(hitStore.#hitChance);
		for (const [dmg, p] of hitStore.#distribution.entries()) {
			newHitStore.storeSingle(Math.trunc(dmg * multiplier), p);
		}
