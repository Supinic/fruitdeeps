...
export class Flags {
	...
	salveBlackMask () {
		...
		if (attributes.includes("undead") && salvList.includes(neck) && (neck === "Salve amulet(i)" || neck === "Salve amulet(ei)" || this.calcs.vertex === "Melee")) {
			return [neck];
		}
	...
	void () {
		...
		// Check each slot for being a piece of void equipment
	...
	barrows () {
		...
		let dharoks = (this.calcs.vertex === "Melee");
		...
		let ahrims = (neck.includes("Amulet of the damned") && player.spell);
		for (const slot of loadOut) {
		...
		let karils = (neck.includes("Amulet of the damned") && ammo === "Bolt rack");
		for (const slot of loadOut) {
	...
	npcs () {
		...
		if (this.state.monster.name === "Corporeal Beast" && !(this.calcs.vertex === "Magic" || weaponName.includes("spear") || (weaponName.includes("halberd") || weaponName.includes("fang")) && type === "Stab")) {
