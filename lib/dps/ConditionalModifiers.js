...
import * as verzik23 from "./modifiers/verzik-phase2-3.js";
...
export function modifyAttackSpeed (speed, dps, player, monster) {
	...
	for (const mod of list) {
		speed = mod.modifyMaxHit(modifyAttackSpeed, dps, player, monster);
	}
...
export function applyModifier (dps, player, monster) {
	...
	for (const mod of list) {
		resultDpsObject = mod.changeAttackRoll(resultDpsObject, dps, player, monster);
	}
