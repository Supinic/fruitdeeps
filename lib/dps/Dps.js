...
export class Dps {
	...
	setHitDist () {
		...
		this.calcs.hitStore = new HitFreqStore(1).createBasicDistribution(max, acc);

		// This call can possibly mutate the `hitStore` property
		this.applyConditionals();

		const { hitStore } = this.calcs;
		const distribution = hitStore.getFinalDistribution();

		// Clamp damage to monster's maximum HP, if it is exceeded
		const hp = this.state.monster.stats.hitpoints;
		if (hitStore.maxHit > hp) {
			for (const [dmg, p] of distribution.entries()) {
				if (dmg > hp) {
					distribution.set(hp, distribution.get(hp) + p);
					distribution.delete(dmg);
				}
			}
		}
