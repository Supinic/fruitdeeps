...
export class Dps {
	...
	setFlags () {
		const flags = new Flags(this.state, this.calcs);
	...
	setAccuracy () {
		...
		if (this.calcs.flags.includes("Maiden freeze")) {
			const minRoll = acc.generalFormula(this.state.player.stats.magic + 9, 0);
			const maxRoll = acc.generalFormula(this.state.player.stats.magic + 9, 140);
