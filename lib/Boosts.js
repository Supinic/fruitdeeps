import boosts from "../game-data/boosts.json" assert { type: "json" };
...
export const determineBoostStats = (boostName) => {
	const boostData = boosts.find(i => i.name === boostName);
	if (!boostData) {
		throw new Error(`Invalid boost ${boostName}`);
	}

	return Object.keys(boostData.effects);
};
